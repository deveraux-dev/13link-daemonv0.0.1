...
fn main() {
    ...
    // Standalone mirror (this repo) sits beside F:\v3, not under its crates/ —
    // fall back to the real crates dir when the sibling assumption doesn't hold.
    let sibling = manifest_dir.parent().expect("has a parent").to_path_buf();
    let crates_dir = if sibling.join("forge-foreman-v3").is_dir() {
        sibling
    } else {
        PathBuf::from("F:/v3/crates")
    };
